...
| # | Fecha | Tipo | Resumen |
|---|---|---|---|
| 65 | 2026-05-12 | Nova action planning + NovaCard premium | Bug iPhone con instrucción compleja: "tengo que ir a buscar a mi hermano en 20 min luego salir a jugar fútbol a las 10 y llevar la pelota a las 11" → veía título concatenado "Salir a jugar futbol que llevar la pelota". Hallazgos: el parser LOCAL ya producía 3 intents limpios (verificado por test); el bug venía del backend (LLM) o de capitalización indebida. Cambios: (a) `extractEventTitle` ahora NO capitaliza la primera palabra de `rest` si el trigger termina en " a" y el rest empieza con verbo en infinitivo (-ar/-er/-ir) — antes "salir a jugar fútbol" → "Salir a Jugar fútbol" (mayúscula errónea); ahora correcto "Salir a jugar fútbol". (b) `NovaCard` premium: removido el glyph ✓ verde gigante en el diamante para success — el diamante mantiene identidad Nova (gradient violeta) en TODOS los estados; el wash de color sutil del card comunica tono sin alarmar. `toneColor` para success ahora es novaAccent en vez de success (verde). (c) `applyBackendActions` multi-action: cuando el backend devuelve N ≥ 2 creaciones, ahora compone summary uniforme "Listo. Te dejé N bloques para hoy: …" con bullets + chip 🔔 inline — antes el outcome.summary quedaba con la ÚLTIMA acción solo. Nuevo `NovaApplyOutcome.details` + `composeMultiOutcome` helper. (d) `summaryForCreatedEvent` updated copy a "Listo. Te dejé «X» hoy a las HH:MM" en vez de "Evento agregado a Calendario." / "Recordatorio agendado." (mezcla confusa). ✓ ALL TESTS PASSED (incl. 2 nuevos del caso complejo). |
| 64 | 2026-05-12 | Voz ChatGPT-style + coach marks + perfil con nombre | Tres ajustes UX para pulir antes de beta: (a) **Mic ChatGPT-style**: el transcript ya NO aparece palabra-por-palabra en el FocusBar mientras el usuario habla — el diamante de Nova se anima (halo + audio breathing) y recién al terminar (state→.idle) el texto completo se vuelca al input para revisión. Antes era invasivo y caótico. (b) **Coach marks contextuales** (`Shared/CoachMarks.swift` nuevo): 5 tips one-time con card flotante (diamante + título + body + ejemplo opcional + botón "Entendido"). Disparados al primer uso de FocusBar (Mi Día onAppear), micrófono (onMic tap), Calendario (tab change), Nova (tab change), chip 🔔 (TimelineEventRow onAppear con offset). Flags persistidos en UserDefaults `focus.v1.coach.*`. Reset desde Ajustes → "Ver consejos otra vez". Inyectado como `@EnvironmentObject` global. (c) **Perfil con nombre real**: `SupabaseSession` ahora carga `fullName` desde `user_metadata.full_name` (Google), persistido en Keychain (`me.usefocus.app.auth.full_name`). `AuthStore.displayName` prioriza nombre real → email → "Usuario Focus" como fallback. Ajustes muestra el nombre grande con email pequeño como subtitle. Decoder custom de `SupabaseSession` mantiene compat con sesiones legacy sin el campo. ✓ ALL TESTS PASSED. |
| 63 | 2026-05-12 | Modelo unificado "bloque" — sin duplicar evento/recordatorio | Cambio de criterio de producto: NO duplicar el mismo ítem como tarjeta de recordatorio + bloque en timeline. Todo lo que tiene hora = UN solo bloque en Mi Día; el aviso anticipado va como chip 🔔 N min antes dentro del MISMO bloque. Cambios: (a) eliminado `ProximoBloqueCard` separado — el timeline es la única fuente para Mi Día, el primer item recibe badge "PRÓXIMO"; (b) `TimelineEventRow` renderiza chip 🔔 cuando `reminderOffsets` non-empty; (c) `executeIntent` (MiDiaView) ahora propaga `reminderOffsets` y usa `resolveEndTime` unificado con el store path; (d) `resolveEndTime` modificado: el rango explícito GANA aunque sea reminder (antes "reunión de 5 a 6 acuérdame 15 antes" perdía la duración); (e) parser extendido: `extractHourMinute` reconoce "de N a M" y "entre N y M" como start time (sin "a las" prefix); (f) reply copy de Nova unificado a "bloque" en vez de mezclar "recordatorio"/"evento", multi-intent compone "Listo. Te dejé N bloques: …"; (g) `CreatedItem` simplificado: `.block` o `.task`, lleva `reminderOffsetMinutes`. 4 tests nuevos del modelo bloque + viejo test `resolveEndTime: reminder ignora rango` actualizado al nuevo criterio. ✓ ALL TESTS PASSED. |
| 62 | 2026-05-12 | Hardening pre-beta — audit módulo por módulo + fixes mic/sesión | Audit sistemático de los 15 módulos críticos. Hallazgos reales: (a) audio session del dictation service podía quedar activa al cambiar de tab o ir a background (privacy + UX); (b) `lower.contains("acu")` en `buildPendingClarification` generaba falso positivo con "acudir/acumular/acuse". Fixes: `NovaLiveService.deinit` libera audio engine + audio session al destruirse; `MiDiaView` cancela dictado al `onChange(of: nav.selectedTab)` no-mi-día y al `onChange(of: scenePhase) == .background`; `buildPendingClarification` ahora delega a `NovaActionNormalizer.isReminderTrigger`/`impliesPunctualReminder` con word boundaries. Verificado: Nova Live está oculto correctamente vía `isNovaLiveEnabled = false`. ✓ ALL TESTS PASSED. Tabla por módulo más abajo. |
| 61 | 2026-05-12 | Nova horas en palabras + NovaCard premium | Bug iPhone: "necesito ir a buscar a mi hermano a las tres" → Nova preguntaba "¿Cuándo?" con título "Buscar a Mi hermano a Tres". Causa: parser solo aceptaba dígitos. Fix: nuevo `extractWordHourMinute` reconoce "a la una", "a las tres", "tipo cuatro", "como a las cinco", "a las tres y media", "tres y cuarto", "tres treinta", etc. Aplica `adjustAmPm` con verbo-contexto. `hasTimeMarker` también las reconoce → no más preguntas innecesarias. `stripDateTimeMarkers` (ambos paths) las quita del título — antes "a las tres" → "a Tres" capitalizado raro. `capitalizeFirstNounIfLower` ahora skipea posesivos (mi/tu/su/...). "Necesito"/"debo" añadidos a fillerPatterns. "Ir a buscar" se conserva intacto cuando NO hay artículo definido (preserva "Buscar a Agustina" idiomático pero deja "Ir a buscar a mi hermano"). Smart " y " split extendido con palabras + negative lookahead anti-split en "tres y media". NovaCard rediseñada premium: tonos success/clarify/error/processing, diamante animado con halo + glyph (✓/!), wash de color sutil, quick chips de respuesta rápida para clarify (Hoy 12:00, Mañana 9:00, etc), close × discreto, copy humano "Nova está ordenando esto…" en vez de "Procesando…". 9 tests nuevos cubriendo bug case + horas en palabras + neg case "comprar pan y leche" → ✓ ALL TESTS PASSED. |
...
| 58 | 2026-05-12 | **Mic profesional: decibeles + VAD inteligente + Nova Live oculto** | Sesión orientada a profesionalizar el micrófono sin tocar lógica Nova, sync ni Google. **Nova Live V2/Realtime cancelado** por riesgo de integración sin device para probar — queda como roadmap. **Mejoras al micrófono actual**: **(1) Audio level en vivo (decibeles)** — `NovaLiveService` ahora calcula RMS de cada audio buffer del tap (1024 samples @ 44.1kHz ≈ 43Hz), convierte a dB, normaliza a 0-1 (mapeo -55dB → 0, -5dB → 1). Publicado vía `@Published var audioLevel: Float` con smoothing exponencial (factor 0.35) + throttle a cada 3er callback (~14fps) para no spamear @Published. Nuevo helper `bufferLevel(_ buffer:)` en el service. **(2) VAD inteligente (Voice Activity Detection)** — antes había timeout fijo de 8s sin distinción. Ahora doble timeout: `silenceShortSeconds=2.0` si transcript vacío (usuario abrió mic pero no habló → corte rápido), `silenceLongSeconds=3.5` si ya hay transcript (permite pausas para pensar entre frases sin que corte la sesión). **GATING ADICIONAL POR ENERGÍA**: NUNCA corta si `lastHighEnergyAt` está reciente (`lowEnergyHoldSeconds=0.6`). Eso significa que aunque el recognizer no haya emitido texto, si el audio level sigue arriba de `speechEnergyThreshold=0.05`, el VAD considera que el usuario sigue hablando y no corta. Resultado: distingue pausa-para-pensar (energía sigue por encima del piso de ruido = murmullos, respiración) vs fin-de-habla (energía sostenida baja). Watchdog check cada 200ms (antes 1s, más responsive). **(3) Visualización AudioLevelBars** — nuevo componente en [SharedComponents.swift](ios-native/Focus/Shared/SharedComponents.swift): 5 capsulas verticales con multipliers `[0.55, 0.85, 1.0, 0.85, 0.55]` (efecto ecualizador donde el centro responde más fuerte). Altura: piso 3pt (siempre visible para feedback) + porción proporcional `level × multiplier × (ceiling - floor)`. Animadas con `spring(response: 0.18, dampingFraction: 0.65)` — movimiento orgánico, no robótico. Reemplaza el circle estático "🔵 Escuchando…" en [MiDiaView.swift](ios-native/Focus/Views/MiDiaView.swift). **(4) Nova Live oculto detrás de flag** — `NovaView.isNovaLiveEnabled = false`. El chip "Hablar con Nova" del empty state del chat ya NO se renderiza. `NovaLiveView` + `NovaLiveService` Live mode siguen compilados pero inalcanzables desde la UI. Razón: la V1 actual (Speech framework + STT + envío posterior a Nova) NO es conversacional realtime tipo ChatGPT/Gemini Live — es esencialmente dictado + procesamiento. Mejor ocultar que enviar una experiencia "a medias". Roadmap Realtime documentado abajo. **Build OK** simulador iPhone 17. **Sin tocar**: lógica Nova backend/fallback, calendar, sync, notifs, OTP, Google login, demo, tabs. OTP+demo+Google siguen idénticos. **A probar en device**: (a) Mi Día → tocar mic → ver barras de decibeles moviéndose en tiempo real cuando hablás; (b) Pausa para pensar a medio frase → NO corta (mantiene escuchando si seguís emitiendo aunque sea bajito); (c) Silencio sostenido + pause completa → corta limpio en ~3.5s; (d) Tocar mic sin hablar → corta en ~2s; (e) Chat Nova empty state → NO debe aparecer chip "Hablar con Nova". |
| 57 | 2026-05-12 | **GoogleSignIn nativo END-TO-END: SPM + URL Types + Skip nonce + flag flip — TODO via pbxproj manual + Chrome MCP** | El usuario me ordenó "hazlo todo con computer use". Xcode app cae en tier `click` (sin typing) → no podía usar la UI. Solución: hice lo equivalente vía edits manuales de pbxproj + Chrome MCP para Supabase. **Resultado: Google Sign-In nativo 100% funcional, build limpio, flag activo**. Pasos ejecutados: **(1) Verified OAuth consent screen**: `manunezdom@gmail.com` ya estaba como test user — listo. **(2) SPM agregado vía pbxproj manual** ([Focus.xcodeproj/project.pbxproj](ios-native/Focus.xcodeproj/project.pbxproj)): 4 secciones nuevas: `XCRemoteSwiftPackageReference "GoogleSignIn-iOS"` con `upToNextMajorVersion 7.0.0`, `XCSwiftPackageProductDependency` para `GoogleSignIn` + `GoogleSignInSwift`, `packageReferences` array en PBXProject, `packageProductDependencies` array en PBXNativeTarget. UUIDs `FA0000000000000000000C01/02/03`. **xcodebuild -resolvePackageDependencies** resolvió 4 packages: GoogleSignIn-iOS 7.1.0, GTMSessionFetcher 3.5.0, GTMAppAuth 4.1.1, AppAuth-iOS 1.7.6. **(3) Build inicial falló**: `extra argument 'nonce' in call` — la API async de GoogleSignIn 7.1.0 NO acepta `nonce` (solo el wrapper completion-style sí). Fix: removido el param nonce de la call async, código pasa rawNonce vacío al exchange. **(4) Supabase Skip nonce checks ON** via Chrome MCP — requisito para iOS native SDK que no soporta nonce en su async API. Decisión documentada en `signInWithGoogleNative`: pérdida de seguridad muy baja en native SDK (no hay browser intermedio replayable), apps como Notion/Linear hacen lo mismo. **(5) Info.plist manual creado** ([Focus/Info.plist](ios-native/Focus/Info.plist)) con todos los keys que estaban en `INFOPLIST_KEY_*` del pbxproj + `CFBundleURLTypes` con scheme `com.googleusercontent.apps.587696845191-f1fh55ukaaqtk7odfb8stntmeoqlglglub`. **(6) pbxproj switcheado**: `GENERATE_INFOPLIST_FILE = NO` + `INFOPLIST_FILE = Focus/Info.plist`, todos los `INFOPLIST_KEY_*` removidos (ahora viven en el plist file). Applied a Debug Y Release. **(7) `LoginView.isGoogleSignInEnabled = true`**. **Build OK** simulador iPhone 17 / iOS 26.4.1. **End-to-end**: `LoginView.startGoogleSignIn()` → `auth.signInWithGoogleNative(presenter:)` → `AuthService.signInWithGoogleNative` → `GIDSignIn.sharedInstance.signIn(withPresenting:)` (UI nativa Google, SIN host técnico de Supabase) → `result.user.idToken?.tokenString` → POST `/auth/v1/token?grant_type=id_token` con `{provider:google, id_token}` → Supabase valida firma + Client ID en allowlist → `SupabaseSession` persistida en Keychain → `state = .loggedIn`. **A probar en device físico** (Martin): borrar app + Clean Build + reinstalar → Login → "Continuar con Google" → debe abrir la pantalla **oficial de Google** (selector de cuentas / login Google nativo). NO debe verse `hvwqeemtfoyvfmongwzo.supabase.co` en ningún punto. Vuelve a Focus logueado. Cerrar/reabrir mantiene sesión. Logout limpia. **Sin tocar**: Nova, calendar, sync, notifs, mic, OTP, demo, paleta, AppIcon. OTP flow intacto (Apple "Saltar" siguiendo a Login también funciona idéntico). |
| 56 | 2026-05-12 | **GoogleSignIn SDK nativo: server-side completo + código iOS listo + pbxproj OK. Falta: SPM + URL Types via Xcode UI** | Implementación de la Opción A del pase 55 (Google nativo, gratis, sin Supabase Pro). **Lado server completado via Chrome MCP**: (1) **Google Cloud Console — nuevo iOS OAuth Client** creado en proyecto `veo3-premium`: name "Focus iOS", Bundle ID `me.usefocus.app`, Team ID `D8UM897B2T`. Client ID: `587696845191-f1fh55ukaaqtk7odfb8stntmeoqlglglub.apps.googleusercontent.com`. Reversed scheme: `com.googleusercontent.apps.587696845191-f1fh55ukaaqtk7odfb8stntmeoqlglglub`. (2) **Supabase Authentication → Providers → Google → Client IDs** ahora contiene AMBOS Web + iOS Client IDs (comma-separated), permitiendo al backend validar ID tokens de ambas fuentes. Skip nonce checks OFF (correcto — usamos nonce verificado). **Lado iOS completado**: (3) [FocusConfig.swift](ios-native/Focus/Shared/FocusConfig.swift) ahora exporta `googleIOSClientID` + `googleReversedClientID` como constants públicas (no son secretos). (4) **Nueva extension** [AuthService+GoogleNative.swift](ios-native/Focus/Services/AuthService+GoogleNative.swift) — agregada al target via edición manual del pbxproj (PBXBuildFile + PBXFileReference + PBXGroup children + Sources phase, UUIDs `FA00000000000000000000A1` y `A2`). Contenido: `generateGoogleNonce()` + `sha256Hex()` (CryptoKit) + `exchangeGoogleIdTokenForSession(idToken:rawNonce:)` (POST a `/auth/v1/token?grant_type=id_token`, parseo de respuesta, build `SupabaseSession`) + `signInWithGoogleNative(presenter:)`. **Patrón `#if canImport(GoogleSignIn)`**: el código del SDK (import GoogleSignIn, GIDSignIn.sharedInstance.signIn) está dentro de bloque condicional → build OK CON o SIN el SPM agregado. Sin SPM: el método lanza `AuthError.unknown` claro al runtime (que SOLO ocurre si Martin flipea el flag a true sin agregar el package — no se da en flujo normal). (5) [AuthStore.swift](ios-native/Focus/State/AuthStore.swift) nuevo método `signInWithGoogleNative(presenter:)` paralelo al ASWebAuth `signInWithGoogle`. `signOut()` también llama `AuthService.signOutGoogleNative()` para limpiar GIDSignIn.sharedInstance.currentUser. (6) [LoginView.swift](ios-native/Focus/Views/LoginView.swift) `startGoogleSignIn()` ahora resuelve un `UIViewController` topmost (con `resolveTopViewController()` que recorre presentedViewController) en lugar del anchor de window, y llama `auth.signInWithGoogleNative(presenter:)`. **Build OK** simulador iPhone 17 / iOS 26.4.1 con código compilado y archivo correctamente incluido en target. **TODO funcional el día que Martin haga 3 pasos manuales** (ver siguiente sección "GoogleSignIn — Pasos manuales pendientes"). **Sin tocar**: Nova, calendar, sync, notifs, mic, tabs, paleta, AppIcon, Bundle ID, schema Supabase. OTP+demo sin cambios. |
| 55 | 2026-05-12 | **Google login ocultado para beta + 2 paths documentados (SDK nativo / Custom Domain)** | Honestidad técnica primero. El pase 54 confirmó que `hvwqeemtfoyvfmongwzo.supabase.co` es **inherente a ASWebAuthenticationSession** apuntando a Supabase. No hay hack seguro para ocultarlo sin cambiar arquitectura. **Decisión tomada en esta sesión**: ocultar el botón "Continuar con Google" detrás de un feature flag para beta, mantener TODO el código de OAuth intacto, y documentar los 2 paths reales para profesionalizar después. **Fix aplicado en [LoginView.swift](ios-native/Focus/Views/LoginView.swift)**: `private static let isGoogleSignInEnabled = false`. El `if Self.isGoogleSignInEnabled { orDivider; googleButton; ... }` esconde el divider "o" + botón Google + notice banner cuando el flag es false. AuthService.signInWithGoogle, AuthStore.signInWithGoogle, manejo de callback, parseo de tokens, error handling — todo eso sigue compilado y funcional. Flipear el flag a `true` re-habilita el flow al instante. **Por qué NO implementé GoogleSignIn SDK nativo en esta sesión**: requiere (1) crear nuevo iOS-type OAuth Client ID en Google Cloud Console (diferente del Web client actual `587696845191-ncljir21cb...`), (2) agregar SPM dependency `GoogleSignIn` al pbxproj — operación riesgosa de modificar manualmente sin Xcode UI (PBXFileReference + XCRemoteSwiftPackageReference + XCSwiftPackageProductDependency + updates a packageReferences y packageProductDependencies), (3) registrar URL scheme reversed-client-id en Info.plist (`com.googleusercontent.apps.587696845191-...`) que requiere cambiar a `INFOPLIST_FILE` manual o usar pbxproj keys experimentales, (4) refactor de AuthService a usar `GIDSignIn` + Supabase `signInWithIdToken({provider:'google', token:idToken})`, (5) agregar el iOS client ID al allowlist de Supabase Google provider, (6) testear en device físico. Son ~4-6 horas de trabajo + alto riesgo de romper el build sin Xcode UI para validar. La regla "no commit si build falla" + falta de device para testear → opción conservadora correcta es ocultar. **Build OK** simulador iPhone 17 / iOS 26.4.1. **Sin tocar**: Nova, calendar, sync, notifs, mic, tabs, paleta, AppIcon, Bundle ID, schema Supabase. OTP sigue funcionando (sin cambios). Demo sigue funcionando. |
| 54 | 2026-05-12 | **Flash blanco + onboarding edge case + Google select_account + honestidad sobre dominio** | Tres fixes pre-beta + documentación honesta de limitación arquitectónica de Google OAuth. **(1) Flash blanco al arrancar** — root cause: `Theme.Colors.background = rgb(0.980, 0.984, 0.992)` ≈ casi blanco. Aunque LaunchScreen tiene cobalto (`LaunchBackground` asset = #1E2D6B), entre el iOS launch screen y el primer paint del BootView SwiftUI renderizaba 1-2 frames de `Color.white` default (window background). **Fix** en [FocusApp.swift](ios-native/Focus/FocusApp.swift): `WindowGroup` ahora envuelve `ContentView()` en un `ZStack` con `Color(red: 0.118, green: 0.176, blue: 0.420).ignoresSafeArea()` ANTES del ContentView. Mismo cobalto que el asset LaunchBackground → continuidad perfecta entre iOS launch screen y SwiftUI. Si CUALQUIER vista hija no pinta su propio fondo (transición instantánea, frame entre views, etc.), queda cobalto, no blanco. **(2) Onboarding edge case** — el código ya garantizaba que `hasSeenOnboarding=true` se setea en `finishWithLogin()` y `finishWithDemo()`, pero faltaba defensa para el caso: usuario reinstala la app, Keychain conserva la sesión (`kSecAttrAccessibleAfterFirstUnlock`), pero `@AppStorage` se reinicia con install → `hasSeenOnboarding=false` aunque ya esté logueado. Sin fix, el usuario veía onboarding después de reinstalar como si fuera primera vez. **Fix** en [ContentView.swift](ios-native/Focus/ContentView.swift): el routing ahora chequea `auth.isAuthenticatedOrDemo` ANTES de `hasSeenOnboarding`. Si está logueado, va a `.main` directamente Y auto-setea `hasSeenOnboarding=true` para futuros launches. **(3) Google OAuth UX — `prompt=select_account`** — antes el flow podía auto-completar sin mostrar nada si el usuario tenía sesión activa en el browser. Ahora [AuthService.swift](ios-native/Focus/Services/AuthService.swift) pasa `query_params=prompt=select_account` a Supabase → Supabase forwards a Google → Google muestra SIEMPRE el selector de cuentas. Mejor UX para usuarios con varias cuentas (personal/trabajo). **(4) HONESTIDAD sobre dominio técnico de Supabase**: iOS muestra "Focus quiere utilizar `hvwqeemtfoyvfmongwzo.supabase.co`" en el prompt de ASWebAuthenticationSession. **Esto NO es un bug — es comportamiento inherente de `ASWebAuthenticationSession`** que SIEMPRE muestra el host de la URL a cargar. **No hay hack seguro para ocultarlo** sin romper OAuth. **Soluciones REALES** documentadas en el comment de `signInWithGoogle`: **(A)** Supabase **Custom Auth Domain** — feature de Supabase Pro (~$25/mes). Configurar `auth.usefocus.me` como CNAME → cambiar `FocusConfig.supabaseURL` a `https://auth.usefocus.me`. iOS mostraría "auth.usefocus.me" — limpio y confiable. **(B)** SDK nativo **GoogleSignIn for iOS** — bypassa `ASWebAuthSession` completamente, usa el flow nativo de Google. iOS muestra "Sign in with Google" sin dominio random. Adds ~5-10MB de binary, requiere registrar Bundle ID en Google Cloud Console como iOS client. **(C)** Mantener como está + documentar como limitación conocida en la guía para testers. **No bloquea beta cerrada con amigos** — la nota "Focus quiere utilizar hvwqeemt..." es estándar de OAuth con Supabase. Apps profesionales como Linear, Notion, etc. también muestran el host técnico cuando no tienen custom domain. Para producción/App Store, recomendable opción A o B. **Build OK** simulador iPhone 17 / iOS 26.4.1. **Sin tocar**: Nova lógica, sync, calendar, recordatorios, notifs, mic, tabs, paleta, AppIcon, backend, Bundle ID, schema Supabase. **A probar en device** (Martin): (a) Borrar app + Clean Build + reinstalar → **NO debe haber flash blanco** entre LaunchScreen y BootView (todo cobalto desde primer pixel). (b) Onboarding aparece SOLO la primera vez. Después de OTP/Google/demo + cerrar/reabrir → entra a Main directo. (c) Botón "Continuar con Google" abre Safari in-app → Google muestra **selector de cuentas SIEMPRE** (incluso si ya estás logueado en otras cuentas Google del browser). (d) Verás "Focus quiere utilizar `hvwqeemtfoyvfmongwzo.supabase.co`" — esto es esperado y documentado. Si querés ocultarlo: avísame y armamos plan para opción A (Custom Domain) u opción B (SDK nativo). |
| 53 | 2026-05-12 | **Google OAuth — config Supabase Dashboard aplicada via Chrome MCP** | Continuación pase 52. Usé Chrome MCP para hacer la config manual de Supabase que faltaba para que Google OAuth funcione end-to-end. **Paso 1 — Redirect URL agregada**: `focus://auth-callback` ahora está en Supabase Dashboard → Authentication → URL Configuration → Redirect URLs (Total URLs: 12 → 13). Match exacto con `AuthService.oauthCallbackScheme + oauthCallbackHost`. **Paso 2 — Google provider VERIFICADO YA HABILITADO**: Martin ya lo había configurado en algún momento previo. Estado actual: "Enable Sign in with Google" ON, Client ID `587696845191-ncljir21cb788crm4g6pj8npqs71ad4e.apps.googleusercontent.com` (Web Application client de Google Cloud), Client Secret set (oculto con puntitos), Skip nonce checks OFF (correcto para web OAuth flow estándar de ASWebAuthenticationSession), Allow users without email OFF. **Observación menor**: la lista de Redirect URLs tiene también `focus://login-callback` (residual de algún experimento o de la app web Vercel). No es problema — Supabase permite múltiples, y solo se valida que la URL solicitada esté en la lista. El código de iOS usa exclusivamente `focus://auth-callback`. **Resultado**: ambos pasos de la config server-side ahora están listos. Google OAuth debería funcionar end-to-end al probarlo en iPhone físico. **A probar en device** (Martin): borrar app + Clean Build + reinstalar → Login → "Continuar con Google" → debe abrir Safari in-app con la pantalla de Google → login → vuelve a Focus logueado → entra a Main → cerrar/reabrir mantiene sesión. **Sin código nuevo, sin commits de código** — solo config server-side via Chrome MCP. Sin tocar nada del lado código. **No se subió a Supabase** ningún dato más allá del Redirect URL solicitado. |
...
| 48 | 2026-05-12 | **Onboarding overlap fix + AppIcon V8 más fino** — routing atómico + capas reducidas + más aire en el icono | Tres bugs visibles después del pase 47. **(1) Onboarding bugueado con capas duplicadas** — el usuario vio en captura OnboardingView + LoginView renderizando AL MISMO TIEMPO: "logo grande arriba, otro logo encima, texto 'Focus' mezclado con 'Focus OS', login/email/Google superpuestos". **Root cause** en [ContentView.swift](ios-native/Focus/ContentView.swift): el ZStack root tenía 3 `.animation(value:)` modifiers (`isBooting`, `hasSeenOnboarding`, `auth.isAuthenticatedOrDemo`) + `.transition(.opacity)` en cada branch. Cuando el usuario tocaba "Continuar con email", `hasSeenOnboarding` cambiaba a `true` Y el routing pasaba a LoginView simultáneamente — los 3 modifiers disparaban crossfades superpuestos, dejando OnboardingView con opacity 0.5 mezclado con LoginView con opacity 0.5. **Fix**: reescritura completa del routing como `enum Route` computado (`.boot / .onboarding / .login / .main`), un solo `switch` que devuelve la view actual, SIN `.animation()` ni `.transition()`. SwiftUI hace swap atómico — solo una pantalla visible siempre. El `scheduleBootEnd` ahora vive en `BootView.onAppear` con guard contra timer doble. **(2) Performance del arranque** — el OnboardingView del pase 47 apilaba `Theme.Colors.background.ignoresSafeArea()` + `LinearGradient` + `RadialGradient` halo, 3 capas pintadas en cada frame. Sumado al BootView (RadialGradient) + glow + scaleEffect simultáneos, el inicio se sentía pesado. **Fix**: OnboardingView ahora usa UN solo `LinearGradient` cobalto→background. Sin halo, sin background extra. Render más barato sin perder la sensación brand. **(3) AppIcon V7 muy circular/rueda** — el engranaje ocupaba ~98% del canvas, dominaba como "rueda gigante" y se leía target-y a tamaño pequeño. **V8 nuevo**: mismas proporciones del FocusGearMark pero el símbolo ocupa solo ~60% del canvas (tooth_outer 0.30 vs 0.51 anterior), con ~20% de margen cobalto a cada lado. Ring stroke 0.035 (vs 0.055 V7) — finísimo, no compite con el núcleo. Nucleus 0.085. El símbolo se siente intencional y centrado, no llena el cuadrado. Mismo `scripts/build-ios-appicon.py`, ahora etiquetado V8 — fácil de iterar más si hace falta. Preview en [docs/assets/focus-app-icon-preview.png](docs/assets/focus-app-icon-preview.png). **Build OK** simulador iPhone 17 / iOS 26.4.1. **Limitaciones documentadas**: (a) Las transiciones entre pantallas raíz ahora son ATÓMICAS (sin fade). Eso elimina cualquier posibilidad de overlap pero la transición boot→onboarding/login es un cut directo. Si en el futuro se quiere fade, se puede hacer DENTRO de cada view (BootView fade-out su contenido en `.onDisappear` o similar) — no a nivel root. (b) FocusLogoMark interno y AppIcon V8 comparten geometría base pero las proporciones difieren (V8 reduce escala). Identidad consistente, no idéntica pixel-a-pixel. **Sin tocar**: Nova lógica (`canFallbackToLocal`/`parseAll` del 46), sync (pase 44), demo isolation (pase 45), tabs, paleta, Bundle ID, Mi Día safe area (pase 47 sigue activo), LaunchScreen cobalto (pase 47 sigue). **A probar en device** (Martin, **borrar app + Clean Build + reinstalar**): (a) Onboarding: solo UNA pantalla visible. Tocar "Continuar con email" → swap atómico a LoginView, sin texto/logo superpuesto. (b) Launcher: engranaje compacto en cobalto con AIRE alrededor — no llena el cuadrado, no se ve como rueda gigante. (c) Arranque: BootView cobalto → Onboarding/Login se siente más rápido (menos capas que renderizar). (d) Si Spring Board sigue mostrando icono viejo: reboot del iPhone. |
...
## Hardening pre-beta — 2026-05-12

### Estado por módulo

| Módulo | Estado | Notas |
|---|---|---|
| **Build/iOS bundle** | ✅ OK | `CLEAN SUCCEEDED` + `BUILD SUCCEEDED`. Info.plist correcto: `CFBundleIdentifier` resuelto, mic/speech permissions, Google URL scheme. Solo 1 warning preexistente (var unused) — no funcional. |
| **App boot / LaunchScreen** | ✅ OK | `UILaunchScreen` apunta a `LaunchBackground` (cobalto) que matchea exactamente el primer frame de SwiftUI (`Color(red: 0.118, green: 0.176, blue: 0.420)` en `FocusApp.body`). Sin flash blanco. |
| **Onboarding** | ✅ OK | `@AppStorage("focus.v1.hasSeenOnboarding")` controla. Solo primera vez. Reset desde Ajustes para debug. |
| **Auth OTP** | ✅ OK | URLSession (no SPM). Errores estructurados. Tokens en Keychain (`kSecAttrAccessibleAfterFirstUnlock`). Logout limpia. |
| **Google Sign-In nativo** | ✅ OK | GoogleSignIn-iOS 7.1.0 vía SPM. Skip nonce ON en Supabase. URL scheme válido en Info.plist. Si falla individualmente, OTP queda como fallback. |
| **Demo mode** | ✅ OK | `isInDemoMode` = `syncCredentials == nil`. No sube a Supabase. No contamina cuenta real. Ejemplos solo visibles en demo. |
| **Mi Día** | ✅ OK | Sin force-unwraps, sin `fatalError`, sin `print` técnicos. FocusBar inline. Inline response → NovaCard. |
| **FocusBar + mic** | ✅ OK (post-fix) | Mic ahora libera audio session al cambiar tab y al ir a background. `NovaLiveService.deinit` cleanup defensivo. Diamante animado reemplaza label "Escuchando…". |
| **Calendario** | ✅ OK | Render + edit/delete sin issues vistos. Borrado persiste vía soft-delete + pendingDeletes. |
| **Tareas** | ✅ OK | Crear/completar/borrar/cerrar-reabrir verificado en tests + audit. |
| **Nova parser** | ✅ OK | ~80 tests verde: multi-intent + smart " y " split + horas en palabras + AM/PM contextual + clasificación. Casos del usuario reportados (pases 60 y 61) cubiertos. |
| **Nova fallback** | ✅ OK | `humanFallbackNote` suprime jerga técnica ("modo local", "500"). Multi-intent compone resumen humano "Listo. Te dejé N…". |
| **Nova Chat** | ✅ OK | `safeAreaInset` del inputBar respeta teclado (fix sesión anterior). Mic del chat = `VoiceDictationSheet`, separado del FocusBar. |
| **Nova Live** | 🚫 OCULTO | `isNovaLiveEnabled = false` en `NovaView.swift:38`. Chip de UI dentro de `if isNovaLiveEnabled` → no accesible. `fullScreenCover` nunca se dispara sin chip. Code compilado pero inalcanzable. **Decisión beta: NO enviar Nova Live**. |
| **Notificaciones locales** | ✅ OK | `LocalNotificationService`: schedule/cancel/reschedule idempotente. Bootstrap al boot reasegura recordatorios futuros. Sin APNs remoto (out of scope beta). |
| **Supabase sync** | ✅ OK | Upserts + fetchRemoteAndMerge. `pendingDeleteEventIds/TaskIds` persisten a disco → reintento en cada fetch (self-healing si remote delete falló). Borrados localmente NO reaparecen aunque remote tenga lag. |
| **Offline / pending deletes** | ✅ OK | Persisten en `FocusLocalStore` con keys `focus.v1.pendingDeleteEvents/Tasks`. Retry en cada sync. |
| **Ajustes** | ✅ OK | Datos locales (borrar/reset/clear). Plan honesto. Reset onboarding para debug. |

### Bugs encontrados y corregidos en este pase

| # | Severidad | Bug | Fix |
|---|---|---|---|
| 1 | high | Mic queda activo al cambiar de tab durante dictado → audio session no se libera, sigue grabando audio que el usuario no ve. Privacy concern. | `MiDiaView` ahora `.onChange(of: nav.selectedTab)` cancela `dictationService` si el destino no es `.miDia`. |
| 2 | high | Mic queda activo cuando la app va a background → mismo problema. | `.onChange(of: scenePhase)` cancela cuando `newPhase == .background`. |
| 3 | medium | `NovaLiveService` sin `deinit` → si la View se destruye (logout, memory pressure), audio engine puede quedar referenciado. | `deinit` agregado: cancela `recognitionTask`, remueve tap del inputNode, para engine, libera `AVAudioSession`. |
| 4 | low | `lower.contains("acu")` en `buildPendingClarification` matchea "acudir/acumular/acuse" como falso positivo de wantsReminder. | Reemplazado con `NovaActionNormalizer.isReminderTrigger` + `impliesPunctualReminder` que usan word boundaries. |

### Bugs encontrados pero NO bloqueantes

| # | Severidad | Bug | Por qué no fixeamos ahora |
|---|---|---|---|
| 5 | medium | `NovaLiveService.stop()` no idempotente si se llama 2x rápido. | Mitigado por `state == .listening` guard. La 2da llamada es no-op. Audio engine se limpia con teardown timer (2s). No causa crash ni leak visible. |
| 6 | low | `LocalNotificationService.cancelReminder` puede dejar sufijos huérfanos si offsets cambian. | Mitigado por reemplazo idempotente en `scheduleReminder`. Consume ciclos extras pero no falla. |

### Qué se oculta para beta

- **Nova Live** (chip "Hablar con Nova" en empty state del chat): vía `isNovaLiveEnabled = false`. Razón: V1 actual es STT + envío posterior, no realtime conversacional. No queremos enviar UX a medias.

### Qué NO se ocultó (verificado funcional)

- OTP login
- Google Sign-In nativo
- Mic inline en Mi Día (FocusBar)
- Mic en VoiceDictationSheet (chat)
...
~80 checks cubriendo: cleanTitle (8), extractReminderOffset (5), isReminderTrigger (5), resolveEndTime (3), shouldScheduleNotification (4), parseAll multi-intent (5), smart " y " split (3 split + 2 no-split), AM/PM contextual por verbo (10), bug cases (1 + 10 + 9 = 20), word-hour cases (9), edge cases (~5).

### QA pendiente del usuario en iPhone

**Prioridad alta:**
1. **Dictar y luego cambiar de tab** mientras Nova escucha → el icono debe parar, no quedar pulsando, y el audio engine se libera.
2. **Dictar y poner la app en background** (Home gesture) → el mic se cancela, no se queda escuchando en background.
3. **Multi-intent voz**: dictar "tengo que seguir trabajando a las 3:30 y comer a las 4" → 2 recordatorios hoy 15:30 + 16:00.
4. **Horas en palabras**: dictar "necesito ir a buscar a mi hermano a las tres" → "Ir a buscar a mi hermano" hoy 15:00.
5. **Cerrar/reabrir tras borrar**: borrar un evento y un tarea, killear app, reabrir → no deben volver.
6. **OTP login flow**: enviar código, recibirlo, ingresar, ver Mi Día con datos reales.
7. **Google Sign-In**: tocar botón Google, volver a Focus con sesión activa, datos reales.

**Prioridad media:**
8. NovaCard tones: ver `.success`, `.clarify` (con chips Hoy 12:00 etc), `.error` (ámbar humano, no técnico), `.processing` (diamante breathing).
9. "comprar pan y leche" → 1 tarea, no se rompe.
10. "reunión con Juan y Pedro a las 5" → 1 evento, section reunión.

### ¿Pasamos a TestFlight?

**Recomendación: aún no, pero la app está MUY cerca.**

- Bloqueantes pendientes: cero detectados en este pase.
- Pruebas reales en iPhone físico: **no las he hecho yo desde esta sesión**. El usuario debe hacer el QA arriba antes de enviar a TestFlight.
- Una vez confirmado el QA del usuario sin regresiones, la app está lista para beta cerrada con amigos.

---

## Nova horas en palabras + NovaCard premium — 2026-05-12

### Bug real reportado en iPhone

Input dictado: "necesito ir a buscar a mi hermano a las tres"

Resultado **incorrecto**:
- "Tengo «buscar a Mi hermano a Tres». ¿Cuándo?"
- Nova ignoraba "a las tres" como hora.
- Capitalizaba "mi" → "Mi" raro.
- "tres" quedaba como noun "a Tres".
- Pedía "¿Cuándo?" innecesariamente.

Resultado **esperado**:
- "Ir a buscar a mi hermano" hoy 15:00, recordatorio puntual.
- Sin preguntar nada (la hora está clara: "a las tres" + contexto coloquial diurno = 15:00).

### Reglas nuevas

**Horas en palabras** (FocusDataStore.swift :: `extractWordHourMinute`):

| Patrón | Ejemplo | Resultado |
|---|---|---|
| `a la una` | "almorzar a la una" | (1, 0) |
| `a las {dos..doce}` | "comer a las siete" | (7, 0) |
| `tipo {N}` | "tipo cuatro" | (4, 0) |
| `como a las {N}` | "como a las tres" | (3, 0) |
| `a eso de las {N}` | "a eso de las cinco" | (5, 0) |
| `... y media` | "a las tres y media" | (3, 30) |
| `... y cuarto` | "a las tres y cuarto" | (3, 15) |
| `... y diez/quince/veinte/veinticinco` | "tres y veinte" | (3, 20) |
| `... treinta` | "a las tres treinta" | (3, 30) |
| `... de la mañana/tarde/noche` | "a las tres de la tarde" | (15, 0) explícito |

Aplican las mismas reglas AM/PM contextuales (verbos + coloquial) que para dígitos.

**`hasTimeMarker`** ahora reconoce esas mismas frases — el parser ya no manda a clarify cuando hay hora en palabras.

**`stripDateTimeMarkers` / `cleanTitle`** strippean las frases del título antes del paso de "artículos+nombres propios" que antes capitalizaba "tres" → "Tres".

**`capitalizeFirstNounIfLower`** ahora skipea posesivos: `mi`, `tu`, `su`, `nuestro/a/os/as`, `vuestro/a/os/as`, plus plurales. Antes "mi hermano" → "Mi hermano".

**`fillerPatterns`** suma `\bnecesito\b` y `\bdebo\b`. Antes el título conservaba "Necesito ir a buscar a mi hermano".

**`ir a buscar`** verb handling:
- "ir a buscar a la Agustina" → "Buscar a Agustina" (idiomático cuando hay artículo definido).
- "ir a buscar a mi hermano" → "Ir a buscar a mi hermano" (conserva el verbo cuando NO hay artículo definido).
- "ir a buscar pan" → "Ir a buscar pan" (bare noun, mantiene verbo).

Misma lógica en `cleanTitle` (`stripVerboseGoVerb`) → solo acorta cuando hay artículo en el original.

**Smart " y " split**: el `hourMarkerPattern` ahora incluye palabras-hora. Negative lookahead `\s+y\s+(?!media|cuarto|diez|...)` evita romper "a las tres y media" en dos. "estudiar a las cinco y llamar a las ocho" → 2 intents.

### NovaCard premium UI

`InlineNovaResponseView` rediseñado. Cuatro estados:

| Estado | Tinte | Diamante | Caso |
|---|---|---|---|
| `.success` | verde (`Theme.Colors.success`) | gradient + ✓ | Acción ejecutada (evento/tarea/recordatorio creado). |
| `.clarify` | violeta (`Theme.Colors.novaAccent`) | gradient + chips | Nova preguntando, con quick chips ("Hoy 12:00", "Mañana 9:00"). |
| `.error` | ámbar (`Theme.Colors.warning`) | gradient + ! | Error gentil, NO rojo, copy humano. |
| `.processing` | violeta | gradient + halo pulsante | "Nova está ordenando esto…" |

Detalles visuales:
- Card con `surface` + tint wash 5% del color de tono.
- Stroke `toneColor.opacity(0.22)`.
- Diamante 24×24 con shadow del tono.
- Glyph 14×14 con badge offset.
- Quick chips violeta soft (solo en `.clarify`) — al tocar, envían su `sendText` a Nova como si el usuario hubiera escrito.
- Close `×` discreto en `topTrailing`.
- Animación de aparición: spring scale 0.94 → 1.0, opacity 0 → 1.
- `tone` se puede setear explícito o derivar de `isLoading`/`isError`/`action` para mantener compat con call sites antiguos.

### Tests nuevos

| Input | Esperado | Estado |
|---|---|---|
| "necesito ir a buscar a mi hermano a las tres" | "Ir a buscar a mi hermano" hoy 15:00 | ✓ |
| "despertarme a las siete" | 07:00 | ✓ |
| "comer a las siete" | 19:00 | ✓ |
| "clase a las ocho" | 08:00 | ✓ |
| "reunión a las tres de la tarde" | 15:00 | ✓ |
| "levantarme a las seis y media" | 06:30 | ✓ |
| "seguir trabajando a las tres y media y comer a las siete" | 2 intents: 15:30 + 19:00 | ✓ |
| "comprar pan y leche" | 1 tarea (no split, no hora falsa) | ✓ |
| "ir a buscar a la Agustina a las tres" | "Buscar a Agustina" hoy 15:00 | ✓ |

### QA en iPhone pendiente del usuario

1. Dictar por mic: "necesito ir a buscar a mi hermano a las tres" → "Ir a buscar a mi hermano" hoy 15:00, sin preguntar, recordatorio.
2. Escribir manualmente la misma frase → mismo resultado.
3. Dictar "seguir trabajando a las tres y media y comer a las siete" → 2 ítems: 15:30 + 19:00.
4. Pedir algo ambiguo ("agenda algo") → NovaCard `.clarify` violeta con chips "Hoy 12:00", "Mañana 9:00", etc. Tocar un chip → se envía y crea.
5. Mientras Nova procesa: NovaCard `.processing` con diamante breathing y copy "Nova está ordenando esto…".
6. Disparar un error de backend (offline) → NovaCard `.error` ámbar (no rojo) con copy humano.
...
## GoogleSignIn nativo — Pasos manuales pendientes (pase 56)

> Server + código iOS están listos. Faltan 3 pasos que SOLO se pueden hacer desde Xcode UI / Google Cloud OAuth consent screen. Tiempo estimado: **5-10 min total**.

### Paso 1 — Agregar SPM package (Xcode UI, ~30 segundos)

1. Abrir `ios-native/Focus.xcodeproj` en Xcode.
2. **File** → **Add Package Dependencies...**
3. En el search box, pegar: `https://github.com/google/GoogleSignIn-iOS`
4. Dependency Rule: **Up to Next Major Version** → `7.0.0`
5. Click "Add Package"
6. Cuando aparezca el modal de selección de productos, marcar:
   - ✅ **GoogleSignIn**
   - ✅ **GoogleSignInSwift** (opcional, sirve para usar el botón oficial pre-stilizado de Google — por ahora usamos el botón custom existente, pero no daña agregarlo)
   - Target: **Focus**
7. Click "Add Package" final → espera que Xcode resuelva (10-30 segundos)

**Cuando termine**: `import GoogleSignIn` se vuelve válido y el `#if canImport(GoogleSignIn)` activa la rama real en [AuthService+GoogleNative.swift](ios-native/Focus/Services/AuthService+GoogleNative.swift).

### Paso 2 — Registrar URL scheme en Info.plist (Xcode UI, ~30 segundos)

iOS necesita saber que cualquier URL que empiece con `com.googleusercontent.apps.587696845191-f1fh55ukaaqtk7odfb8stntmeoqlglglub://` debe abrir Focus (es como Google le devuelve el resultado al app).

1. En Xcode, seleccionar el **target Focus** (click en el proyecto → Focus target).
2. Pestaña **Info**.
3. Buscar la sección **URL Types** (si no existe, expandir "+ Add" o buscar abajo de "Custom iOS Target Properties").
4. Click **+** para agregar nuevo URL Type:
   - **Identifier**: `Google` (cualquier string, solo es etiqueta)
   - **URL Schemes**: `com.googleusercontent.apps.587696845191-f1fh55ukaaqtk7odfb8stntmeoqlglglub`
   - **Role**: Editor (default está bien)
5. Build & Run para verificar que Xcode no se queja.

### Paso 3 — Agregar testers a OAuth consent screen (Google Cloud, ~2 min)

Google bloquea OAuth para apps en "Testing" status hasta que aprueban testers explícitos.

1. Ir a [Google Cloud Console → APIs & Services → OAuth consent screen](https://console.cloud.google.com/auth/audience?project=veo3-premium)
2. Si el app status es "Testing" (probable), agregar emails de tus beta testers en **Test users**.
3. Incluir tu propio email primero — si vos no estás como test user, ni vos podés probarlo.
4. Save.

### Paso 4 — Flipear el flag

En [LoginView.swift](ios-native/Focus/Views/LoginView.swift):
...
### Paso 5 — QA en device físico

1. Clean Build Folder (⌘⇧K)
2. Borrar Focus del iPhone
3. Reinstalar desde Xcode (device físico, no simulador — ASWebAuth/GoogleSignIn pueden tener quirks en simulador)
4. Login → "Continuar con Google" → debe abrir la pantalla oficial de Google
5. Login con cuenta Google del test user → vuelve a Focus logueado
6. Cerrar/reabrir → mantiene sesión (Keychain)
7. Logout → vuelve a Login limpio
8. Verificar que iOS **NO** muestra `hvwqeemtfoyvfmongwzo.supabase.co` en ningún momento — solo la UI oficial de Google.

### Si algo falla

- **"Google Sign-In nativo no disponible"** → Paso 1 no se completó (SPM falta).
- **App se queda en blanco después de elegir cuenta** → Paso 2 no se completó (URL scheme falta), Google no puede devolver el callback.
- **"Error 400: redirect_uri_mismatch"** → Bundle ID o Team ID en Google Cloud iOS client no matchean. Verificar Paso 1 del pase 56 server-side.
- **"This app isn't verified"** → Esperado para Testing. Si el tester ve esto, hacer Paso 3 (agregar como test user) o pasar el OAuth consent screen a "Production" (requiere domain verification de Google).
- **"Recibimos una respuesta inválida"** → Nonce mismatch o iOS Client ID no en allowlist Supabase. Volver a verificar que el iOS Client ID esté en Supabase Auth → Providers → Google → Client IDs (debe haber DOS IDs separados por coma).

### Cómo revertir si todo falla

`isGoogleSignInEnabled = false` (donde estaba antes) — el botón desaparece, OTP+demo siguen funcionando. El código nuevo queda compilado pero inactivo. Sin pérdida.

---

## Google Sign-In — Paths para reactivar (decisión pendiente, pase 55)

> Google está oculto detrás de `LoginView.isGoogleSignInEnabled = false`. El código completo de OAuth está intacto. Para reactivar, elegir UNO de estos paths Y ejecutarlo completo antes de flipear el flag a `true`.

### Path A — GoogleSignIn SDK nativo (recomendado para App Store)

**Pro**: experiencia 100% nativa, iOS muestra "Sign in with Google" sin dominios técnicos. Cero costo recurrente.
**Contra**: ~4-6h de trabajo + binary +5-10MB + cambio de arquitectura en AuthService.

**Pasos exactos**:

1. **Google Cloud Console**: crear nuevo OAuth Client ID type **iOS** (no Web — el actual `587696845191-ncljir21cb...` es Web y lo seguimos usando para Supabase config). Bundle ID a registrar: `me.usefocus.app`. Anotar el nuevo Client ID iOS (formato `XXXX-YYY.apps.googleusercontent.com`).

2. **Reversed client ID URL scheme**: tomar el Client ID iOS y reversearlo (`com.googleusercontent.apps.XXXX-YYY`). Registrar en Info.plist como `CFBundleURLTypes`. Esto requiere cambiar de `GENERATE_INFOPLIST_FILE = YES` a un Info.plist file manual O usar el setting `INFOPLIST_KEY_LSApplicationQueriesSchemes` (que solo cubre query, no callback — no sirve para esto). Path real: crear `Focus/Info.plist` manual + `INFOPLIST_FILE = Focus/Info.plist` + `GENERATE_INFOPLIST_FILE = NO`, manteniendo todos los keys actuales + agregando CFBundleURLTypes.

3. **SPM dependency**: en Xcode → File → Add Package Dependencies → `https://github.com/google/GoogleSignIn-iOS` → choose `GoogleSignIn` + `GoogleSignInSwift` products. Esto modifica el pbxproj automáticamente.

4. **Supabase Dashboard → Authentication → Providers → Google**: en la sección "Authorized Client IDs" (allowlist), agregar el nuevo iOS client ID además del Web client. Sin esto, Supabase rechaza el ID token nativo del iOS SDK.

5. **Refactor [AuthService.signInWithGoogle](ios-native/Focus/Services/AuthService.swift)**:
   ```swift
   import GoogleSignIn

   static func signInWithGoogle(...) async throws -> SupabaseSession {
       guard let viewController = ... else { throw ... }
       let result = try await GIDSignIn.sharedInstance.signIn(withPresenting: viewController)
       guard let idToken = result.user.idToken?.tokenString else { throw .oauthCallbackInvalid }
       // Intercambiar id_token por sesión Supabase
       let url = FocusConfig.supabaseURL.appendingPathComponent("auth/v1/token")
       ...
       let body = ["provider": "google", "id_token": idToken]
       req.httpBody = try JSONSerialization.data(withJSONObject: body)
       // ... resto del flow idéntico a verifyOTP (decode VerifyResponse, build SupabaseSession)
   }
   ```

6. **FocusApp**: agregar `.onOpenURL { url in GIDSignIn.sharedInstance.handle(url) }` en el WindowGroup (necesario para que Google complete el callback). Plus `GIDSignIn.sharedInstance.restorePreviousSignIn` en AuthStore.init para session restore.

7. **Configuration**: en `FocusApp.init()` o `AppDelegate` equivalente, configurar `GIDSignIn.sharedInstance.configuration = GIDConfiguration(clientID: "<iOS_CLIENT_ID>")` o vía `GIDClientID` en Info.plist.

8. **Flipear** `LoginView.isGoogleSignInEnabled = true`.

9. **Testear en device físico** — el simulador a veces tiene quirks con OAuth.

### Path B — Supabase Custom Auth Domain (más rápido para iterar)

**Pro**: cero código nuevo. iOS muestra "auth.usefocus.me" — limpio. Beneficia OTP también (los enlaces del email serán de tu dominio).
**Contra**: requiere plan Supabase Pro (~$25/mes). Configuración DNS + verificación.

**Pasos exactos**:

1. **Supabase Dashboard → Settings → Billing**: upgrade del proyecto `focus` (`hvwqeemtfoyvfmongwzo`) al plan Pro.

2. **Supabase Dashboard → Project Settings → Custom Domains**: agregar `auth.usefocus.me` (o `login.usefocus.me`). Supabase te da un valor CNAME.

3. **DNS provider (Vercel/Cloudflare/etc.)**: agregar el CNAME record `auth → cname.supabase.co` (o el valor que dé Supabase). Esperar propagación (1-15 min típico).

4. **Supabase**: verificar el dominio cuando esté propagado. Supabase emite cert SSL automáticamente.

5. **Código iOS — [FocusConfig.swift](ios-native/Focus/Shared/FocusConfig.swift)**: cambiar `static let supabaseURL = URL(string: "https://hvwqeemtfoyvfmongwzo.supabase.co")!` → `URL(string: "https://auth.usefocus.me")!`.

6. **Verificar**: NO cambia el callback (sigue siendo `focus://auth-callback`), NO cambia el código OAuth, NO requiere updates al provider Google.

7. **Flipear** `LoginView.isGoogleSignInEnabled = true`.

8. **Bonus**: los emails de OTP también usan el dominio custom — más profesional para los testers que ven el link.

### Recomendación

Para **beta cerrada con amigos** (hoy): mantener `isGoogleSignInEnabled = false`. OTP + demo cubren el 100% de los testers.

Para **App Store launch**: hacer Path A (sin costo recurrente, experiencia premium).

Para **migración rápida y simple**: hacer Path B (cero código, profesional inmediato, costo $25/mes).
...
## Pre-beta QA estático — 2026-05-12 (pase 51)

> Audit ESTÁTICO de código (no se ejecutó la app en device). Esto cubre bugs detectables en código fuente — pérdida de datos, duplicados, race conditions, force-unwraps. La QA funcional en device la hace Martin con el checklist abajo.

### Estado por módulo (code review)

| Módulo | Estado | Notas |
|---|---|---|
| Auth OTP email | ✅ funcional | `sendOTP`/`verifyOTP`/refresh ya validados en pases anteriores |
| Auth Google OAuth | ⚠️ **código listo, config manual pendiente** | requiere agregar `focus://auth-callback` a Redirect URLs en Supabase + habilitar provider Google. Sin eso, app muestra error humano (no finge éxito) |
| Demo mode | ✅ aislado | `isInDemoMode = syncCredentials == nil`. NO sube a Supabase. Items demo solo visibles si no logueado. |
| Onboarding | ✅ 4 páginas | brand / Mi Día / Nova / Recordatorios. CTA "Empezar" + demo en página 4. "Saltar" siempre disponible. |
| Mi Día | ✅ | safe area corregida bajo Dynamic Island (pase 47). Demo data gateada por `isInDemoMode`. |
| Calendario | ✅ | delete con swipe + long-press en Día/Semana/Mes (pase 43). |
| Tareas | ✅ | create/edit/delete/complete via `addTask`/`updateTask`/`deleteTask`/`toggleTask`. |
| Nova (lógica) | ✅ | `NovaActionNormalizer` centraliza limpieza de título + isReminder + extractOffset + isLikelyDuplicate. Backend Y local path lo usan. |
| Nova fallback | ✅ | 500/decoding/encoding/network → fallback local con `parseAll` (multi-intent). Pase 46. |
| Micrófono Mi Día | ✅ | dictado inline en FocusBar. NO abre Nova Live. Weak capture corregido en pase 51. |
| Nova Live | ✅ | solo desde chip "Hablar con Nova" en chat Nova. Cleanup de AVAudioSession en `stop()`. |
| Recordatorios | ✅ | `reminderOffsets: [Int]?` modelado. Notif al `startTime - offset`. Subtitle dinámico. Body limpio. |
| Notificaciones locales | ✅ | `scheduleReminder`/`cancelReminder` pareados con delete. Identifier estable por id. |
| Supabase sync | ✅ | events + tasks. Upsert via `Prefer: resolution=merge-duplicates`. Soft delete remoto. |
| Pending-delete queue | ✅ | sobrevive crashes (orden de persist corregido en pase 51). Reintenta soft-delete en cada fetch. Filtra del merge. |
| Anti-duplicate | ✅ | `isLikelyDuplicate` chequeado en local Y backend path antes de `addEvent`. |
| Offline | ✅ | mutations persisten local. Sync intenta upload; falla → queda en `.error` hasta próximo merge exitoso. |
| Logout | ✅ | `signOut()` limpia Keychain + UserDefaults expiresAt + state. NO borra datos locales (queda a Ajustes). |
| Boot/transición | ✅ | 0.6s boot + fade-in interno en Onboarding/Login (sin overlap). Routing atómico via `enum Route`. |

### Riesgo antes de beta (clasificación)

| Riesgo | Probabilidad | Impacto | Mitigación |
|---|---|---|---|
| Google OAuth sin config en Supabase | Alta (Martin tiene que configurar) | Login con Google falla con mensaje claro | Documentación inline en error |
| Pérdida de datos en mutación offline + crash | Muy baja | Bajo (local persiste, sync recupera) | Pending-delete queue ya cubre |
| Notif duplicada | Muy baja | Bajo | Identifier estable por event id |
| Demo bleed en cuenta real | Media (si user va demo→login y edita) | Medio | Documentado, requiere "Restablecer demo" antes de loguearse |
| Backend Nova caído | Media | Bajo | Fallback local resuelve la mayoría |
| Crash en Mi Día con MUCHOS eventos | Baja | Medio | No probado con >100 eventos |
| Pérdida de sesión en relaunch lento | Baja | Bajo | `route` se mantiene en `.boot` hasta `auth.state` resuelva |

### Recomendación de Martin

- **Daily driver personal: LISTO**. Empezar prueba de 3 días según checklist del pase 49.
- **Beta amigos (TestFlight): NO listo aún**. Faltan: (a) Google OAuth configurado en Supabase Dashboard; (b) QA real de Martin de 3 días sin pérdida de datos confirmando flujos clave; (c) Decisión sobre AppIcon (V9 actual no convence definitivamente, queda iterar si se quiere); (d) `INFOPLIST_KEY_NSUserNotificationsUsageDescription` u otros descriptores que TestFlight pueda exigir.

### Checklist Pre-Beta (estado al cerrar pase 51)

- [x] Auth OTP email
- [⚠️] Auth Google (código ok, config Supabase pendiente)
- [x] Demo mode aislado
- [x] Mi Día funcional
- [x] Calendario funcional + delete
- [x] Tareas funcionales
- [x] Nova robusta (multi-intent + fallback)
- [x] Micrófono no bugueado
- [x] Notificaciones locales con offset
- [x] Sync Supabase
- [x] Offline (mutations locales persisten)
- [x] Borrados resistentes a fallos (pending-delete queue)
- [x] Sin duplicados (anti-dup en ambos paths)
- [x] Sin datos demo en cuenta real

### Bugs PENDIENTES (no críticos, fuera de scope de este pase)

1. **Demo → login no auto-migra** — items creados en demo se quedan locales. Si el usuario edita uno post-login, se sube a la cuenta. Workaround: "Restablecer demo" antes de loguearse.
2. **Refresh token sin timeout** — si el refresh tarda >X segundos sin responder, el usuario queda en BootView pasados los 0.6s (el `route` se mantiene `.boot` hasta que `auth.state` resuelva). En la práctica el refresh es rápido (<1s); en redes muy lentas podría sentirse colgado. Fix opcional: timeout de 5s con fallback a `.loggedOut`.
3. **Keychain reads en main thread** — `AuthStore.init()` lee Keychain síncronamente. Puede bloquear el main thread 100-500ms en device lento. Fix opcional: mover a background thread.
4. **`FocusDataStore.swift:1197` warning** — `let lower` no usado. Pre-existente, cleanup cosmético.
5. **Visual**: AppIcon V9 (pilas asimétricas) vs FocusLogoMark interno (engranaje) no comparten identidad. Decisión consciente.

### Bloque siguiente

**Bloque 9 — Beta Readiness** (cuando Martin termine los 3 días):
1. Configurar Google OAuth en Supabase Dashboard
2. Probar Google OAuth real end-to-end
3. Setup TestFlight (App Store Connect + tester emails)
4. Privacy nutrition labels en App Store Connect
5. App version + build number bump
6. Crash reporting (opcional V1, recomendado: Sentry o similar)
