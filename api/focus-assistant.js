...
export default async function handler(req, res) {
  ...
  // ─── DIAG MODE (TEMPORAL — borrar tras QA OpenAI) ─────────────────────────
  // Permite probar el provider OpenAI sin auth Supabase, sin cuotas, sin tocar
  // service_role. Triple gate:
  //   1. VERCEL_ENV !== 'production' (Production NUNCA acepta diag).
  //   2. Body { mode: 'diag-openai' } explícito.
  //   3. Header X-Diag-Secret debe coincidir con NOVA_DIAG_SECRET (env Preview).
  // Si cualquier gate falla, devuelve 401/403. Si pasa, ejecuta solo el flow
  // OpenAI con los messages del body y devuelve raw + adapter sin escribir
  // nada en Supabase.
  if (req.body?.mode === 'diag-openai') {
    if (process.env.VERCEL_ENV === 'production') {
      return res.status(403).json({ error: 'diag_disabled_in_production' })
    }
    const expectedSecret = process.env.NOVA_DIAG_SECRET?.trim() || ''
    const receivedSecret = (typeof req.headers['x-diag-secret'] === 'string'
      ? req.headers['x-diag-secret']
      : '').trim()
    if (!expectedSecret || !receivedSecret || expectedSecret !== receivedSecret) {
      return res.status(401).json({ error: 'invalid_or_missing_diag_secret' })
    }
    return await handleDiagOpenAI(req, res)
  }
...
// ─── DIAG HANDLER (TEMPORAL — borrar tras QA OpenAI) ───────────────────────
// Ejecuta el flow OpenAI puro sobre los messages del body. No usa Supabase,
// no cuenta cuotas, no persiste nada. Defensas de auth/env viven en el
// handler principal — esta función SOLO debe llamarse cuando esos pasaron.
async function handleDiagOpenAI(req, res) {
